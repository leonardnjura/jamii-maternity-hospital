...
import { signinUser } from '../lib/get-users';
...
const Page: NextPageWithLayout<IPageProps> = ({
  ...
  async function handleSignin() {
    ...
    // console.log(`!!fb signin req::${JSON.stringify(reqBody)}`);
    const resBody = await signinUser(reqBody, false);
    const rawData = await resBody.json();
    // console.log(
    //   `!!fb signin res::${JSON.stringify(rawData)} httpStatus::${
    //     resBody.status
    //   }`
    // );

    let someHttpErrorExists = resBody.status != 200;
  ...
  const gotoSignUpWithCredentials = async () => {
    router.push(`${server}/signup`);
  };
  ...
  return (
    <section className={'my-page-full'}>
      <div className="mx-auto text-center pt-10">
        <h1 className="my-page-title">Sign In</h1>
        <div className="text-xs mt-4 mb-4">
          <a
            onClick={() => gotoSignUpWithCredentials()}
