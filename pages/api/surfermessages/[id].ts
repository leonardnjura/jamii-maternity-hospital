...
import { IMessageDataSchema, IUserDataOrCustomData } from '../../../data/types';
import { authenticate } from '../../../services/cipher.service';
...
async function handler(
  ...
  async function getOne(id: string) {
    ...
    try {
      if (opAuthorized) {
        ...
        if (docSnap.exists()) {
          ...
          return res.status(404).json({
            success: false,
            message: `SurferMessage of id ${id} not found`,
          });
        }
      } else {
      ...
      console.log(`Ayayai on fetching one surfermessage fb::${e}`);
